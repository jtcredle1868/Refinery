...
def _is_valid_module(name: str) -> bool:
    return name in {m.value for m in AnalysisModule}
...
def analyze_manuscript(
    ...
    # Validate module names upfront before starting analysis
    if req and req.modules:
        invalid_modules = [m for m in req.modules if not _is_valid_module(m)]
        if invalid_modules:
            raise HTTPException(
                status_code=400,
