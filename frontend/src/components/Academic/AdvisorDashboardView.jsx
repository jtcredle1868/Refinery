    ...
    <div>
      ...
      <div className="flex space-x-1 bg-slate-100 rounded-lg p-1 mb-6 w-fit">
        <button onClick={() => setActiveTab('portfolio')}
          className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'portfolio' ? 'bg-white text-refinery-navy shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
          My Portfolio
        </button>
        <button onClick={() => setActiveTab('students')}
          className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'students' ? 'bg-white text-refinery-navy shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
        ...
        <>
              ...
              <div key={m.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">{m.word_count?.toLocaleString()} words &middot; {m.chapter_count} chapters</p>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${m.status === 'ready' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{m.status}</span>
                </div>
                <div className="flex items-center space-x-2 mt-4 flex-wrap gap-y-2">
                  <Link to={`/manuscript/${m.id}`} className="flex items-center space-x-1 text-sm text-refinery-blue hover:text-blue-700">
          ...
          {manuscripts.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500">No manuscripts yet.</p>
              <Link to="/upload" className="mt-4 inline-block bg-refinery-blue text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">Upload Manuscript</Link>
            </div>
          )}
        </>
      )}
        ...
        <div className="space-y-4">
          {students.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-xl border">
              ...
              <p className="text-slate-500 mb-2">No students linked yet.</p>
              <p className="text-sm text-slate-400">Click "Add Student" above to generate an invite code.</p>
            </div>
          ) : students.map((s) => (
            <div key={s.user_id} className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-semibold text-refinery-navy">{s.full_name || s.email}</h3>
                  <p className="text-xs text-slate-400">{s.email} &middot; {s.manuscript_count} manuscript{s.manuscript_count !== 1 ? 's' : ''}</p>
                </div>
                {s.latest_manuscript && (
                  <div className="text-right">
                    <Link to={`/manuscript/${s.latest_manuscript.id}`} className="text-sm text-refinery-blue hover:text-blue-700 flex items-center space-x-1">
                      <BookOpen className="h-4 w-4" /><span>{s.latest_manuscript.title}</span><ChevronRight className="h-3 w-3" />
                    </Link>
                    <p className="text-xs text-slate-400 mt-1">{s.latest_manuscript.word_count?.toLocaleString()} words</p>
                  </div>
                )}
              </div>
              {s.latest_manuscript && (
            ...
            <div className="p-6">
              {!report ? (
                ...
                <div className="space-y-6">
                  ...
                  {report.executive_summary && <div><h4 className="text-sm font-semibold text-refinery-navy mb-2">Executive Summary</h4><p className="text-sm text-slate-700 whitespace-pre-line">{report.executive_summary}</p></div>}
                  {report.sections?.map((section, i) => <div key={i}><h4 className="text-sm font-semibold text-refinery-navy mb-2">{section.heading}</h4><p className="text-sm text-slate-700 whitespace-pre-line">{section.content}</p></div>)}
                  {report.strengths?.length > 0 && <div><h4 className="text-sm font-semibold text-green-700 mb-2">Strengths</h4><ul className="list-disc ml-5 text-sm text-slate-700 space-y-1">{report.strengths.map((s, i) => <li key={i}>{s}</li>)}</ul></div>}
                  {report.areas_for_improvement?.length > 0 && <div><h4 className="text-sm font-semibold text-amber-700 mb-2">Areas for Improvement</h4><ul className="list-disc ml-5 text-sm text-slate-700 space-y-1">{report.areas_for_improvement.map((a, i) => <li key={i}>{a}</li>)}</ul></div>}
                  {report.recommendations?.length > 0 && <div><h4 className="text-sm font-semibold text-blue-700 mb-2">Recommendations</h4><ul className="list-disc ml-5 text-sm text-slate-700 space-y-1">{report.recommendations.map((r, i) => <li key={i}>{r}</li>)}</ul></div>}
                  {report.overall_assessment && <div className="bg-slate-50 rounded-lg p-4"><h4 className="text-sm font-semibold text-refinery-navy mb-2">Overall Assessment</h4><p className="text-sm text-slate-700">{report.overall_assessment}</p></div>}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setAnnotationModal(null)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b flex items-center justify-between">
              <h3 className="font-display font-semibold text-refinery-navy flex items-center space-x-2">
                <MessageSquare className="h-5 w-5 text-green-500" /><span>Annotations</span>
              </h3>
              <button onClick={() => setAnnotationModal(null)} className="text-slate-400 hover:text-slate-600"><X className="h-5 w-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {annotations.length === 0 && <p className="text-sm text-slate-400 text-center py-8">No annotations yet.</p>}
              {annotations.map((a) => (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setProgressModal(null)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <button onClick={() => setProgressModal(null)} className="text-slate-400 hover:text-slate-600"><X className="h-5 w-5" /></button>
            </div>
            <div className="p-6">
              {progressLoading ? (
                ...
                <div>
                  <h4 className="text-sm font-medium text-slate-600 mb-4">Health Score Over Time</h4>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-400">No progress data yet. Run analyses to build the timeline.</p>
                </div>
              )}
            </div>
          </div>
