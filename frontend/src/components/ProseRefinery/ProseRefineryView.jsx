  ...
  if (!results) {
    return (
      <div className="text-center py-20">
        <p className="text-ink/60">No prose analysis results. Run the Prose Refinery first.</p>
  ...
  return (
    <div>
      ...
      <div className="flex items-center space-x-3 mb-2">
        <PenTool className="h-8 w-8 text-ember" />
        <h1 className="text-3xl font-display text-ink">Prose Refinery</h1>
      </div>
      <p className="text-ink/60 mb-4">Craft-level prose analysis results</p>

      {results.prose_score !== undefined && (
        <div className="border border-ink/10 bg-white/90 rounded-xl p-4 mb-6 inline-block">
          <span className="text-sm text-ink/70">Prose Score: </span>
          <span className="text-2xl font-bold text-ink">{results.prose_score}</span>
      ...
      {/* Tab navigation */}
      <div className="flex space-x-1 bg-ink/10 rounded-lg p-1 mb-6 overflow-x-auto">
        {tabs.map((tab) => (
          <button
            key={tab.key}
      ...
      <div className="rounded-3xl border border-ink/10 bg-white/90 p-6">
              ...
              {(results.tic_tracker.items || []).map((tic, i) => (
                <div key={i} className={`p-4 rounded-lg border ${
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono font-bold text-ink">"{tic.word_or_phrase}"</span>
                    <div className="flex items-center space-x-2">
                      <span className="text-sm text-ink/70">{tic.total_count} occurrences</span>
                  ...
                  <p className="text-sm text-ink/70">{tic.suggestion}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'filter' && results.filter_words && (
          <div>
            <h2 className="font-semibold text-ink mb-2">Filter Word Analysis</h2>
            ...
            {/* Top filter words */}
            <div className="mb-6">
              <h3 className="text-sm font-medium text-ink/80 mb-2">Most frequent filter words</h3>
            ...
            {results.filter_words.chapter_breakdown && (
              <div>
                <h3 className="text-sm font-medium text-ink/80 mb-2">Filter words by chapter</h3>
          ...
          <div>
            ...
            <div className="space-y-4">
              {(results.show_vs_tell.items || []).map((item, i) => (
                <div key={i} className={`p-4 rounded-lg border ${
                  item.severity === 'high' ? 'border-red-200' : 'border-ink/10'
                }`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium text-ink/60">{item.location}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      item.severity === 'high' ? 'bg-red-100 text-red-700' : 'bg-ink/10 text-ink/70'
                    }`}>{item.severity}</span>
                  </div>
                  <div className="bg-red-50 p-3 rounded-md mb-2">
                    <p className="text-sm text-ink/80 line-through italic">"{item.original}"</p>
                  ...
                  <div className="bg-green-50 p-3 rounded-md">
                    ...
                    <p className="text-sm text-ink/80">"{item.suggestion}"</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'rhythm' && results.sentence_rhythm && (
          <div>
            <h2 className="font-semibold text-ink mb-4">Sentence Rhythm Profile</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-ink/5 rounded-lg p-4 text-center">
                <p className="text-2xl font-bold text-ink">{results.sentence_rhythm.avg_sentence_length?.toFixed(1)}</p>
                <p className="text-xs text-ink/60">Avg words/sentence</p>
              </div>
              <div className="bg-ink/5 rounded-lg p-4 text-center">
                <p className="text-2xl font-bold text-ink">{results.sentence_rhythm.variance?.toFixed(1)}</p>
                <p className="text-xs text-ink/60">Variance</p>
              </div>
              <div className="bg-ink/5 rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-ink/60">Rhythm flags</p>
              </div>
            </div>
            ...
            {results.sentence_rhythm.chapter_rhythm_profile && (
              <div>
                <h3 className="text-sm font-medium text-ink/80 mb-2">Rhythm score by chapter</h3>
                <ResponsiveContainer width="100%" height={200}>
            ...
            {(results.sentence_rhythm.consecutive_short_runs?.length > 0 ||
              ...
              <div className="mt-6 space-y-3">
                <h3 className="text-sm font-medium text-ink/80">Flagged rhythm issues</h3>
                {(results.sentence_rhythm.consecutive_short_runs || []).map((r, i) => (
                  <div key={`s${i}`} className="p-3 rounded-lg border border-yellow-200 bg-yellow-50">
                    <span className="text-xs font-medium text-ink/70">{r.location}</span>
                    <span className="text-xs text-yellow-700 ml-2">{r.count} consecutive short sentences</span>
                    <p className="text-sm text-ink/70 mt-1 italic">"{r.excerpt}"</p>
                  </div>
                ))}
        ...
        {activeTab === 'metaphor' && results.metaphor_frequency && (
          <div>
            <h2 className="font-semibold text-ink mb-4">Metaphor Frequency Analysis</h2>
            <div className="space-y-4">
              {(results.metaphor_frequency.metaphor_families || []).map((fam, i) => (
                <div key={i} className="p-4 rounded-lg border border-ink/10">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-ink">{fam.family}</span>
                    <span className="text-sm text-ink/60">{fam.count} occurrences</span>
                  </div>
                  {(fam.overuse_windows || []).filter((w) => w.flagged).length > 0 && (
                    <div className="mt-2">
                      ...
                      {fam.overuse_windows.filter((w) => w.flagged).map((w, j) => (
                        <p key={j} className="text-sm text-ink/70 ml-2">
